...
| Chunk code files with syntax-aware splitting | met | COBOL chunking pipeline in `backend/app/services/cobol_chunker.py`; architecture notes in `docs/architecture.md`. |
| Generate embeddings for all chunks | met | Ingest stats show non-zero chunk counts (`8929`) after full ingest in staging/production (`docs/evaluation-results.md`). |
...
| Implement semantic search across codebase | met | `POST /api/query` returns ranked evidence snippets and citations; sample results in `docs/evaluation-results.md`. |
