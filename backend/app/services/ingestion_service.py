...
class IngestionService:
    ...
    def ingest(self, mode: str = "incremental") -> IngestStats:
        ...
        stats = IngestStats(files_seen=len(files), files_indexed=0, files_skipped=0, chunks_indexed=0)
        collection_ready = False

        for path in files:
            indexed_chunks = self._ingest_file(path=path, mode=mode, collection_ready=collection_ready)
            if indexed_chunks == 0:
                stats.files_skipped += 1
                continue

            stats.files_indexed += 1
            stats.chunks_indexed += indexed_chunks
            collection_ready = True

        return stats

    def _ingest_file(self, path: Path, mode: str, collection_ready: bool) -> int:
